...
### Fixed
- **"FCM credentials rejected" now tells you which of the two causes you hit (#344).** That failure has two opposite explanations — the extracted `AIza…` key is not the FCM one (`API_KEY_SERVICE_BLOCKED`), or the key is fine but Google's Android restriction rejected the request (`API_KEY_ANDROID_APP_BLOCKED`) — and the library the integration uses collapses both into one error and discards Google's reason. The warning therefore asserted the first explanation, sending users who already had the right key off extracting more keys. The integration now asks Google directly why it refused, on the failure path only, and logs the reason with advice matching it: change the key in the first case, check the app label chosen during setup in the second (it determines the package the request identifies itself as, and an unmapped co-brand sends none at all). The general message no longer picks a side.
